...
**Status:** Approved provider and tenant mapping; implementation pending spec review
...
| Service M2M | Clerk M2M | `expense-app-internal` or `expense-foundry-internal` | AI worker to App API/Foundry | machine subject, client/authorized-party ID, `sid`/`jti`, scopes |

The exact Clerk claim names are configured and verified in contract tests before
production use. The verifier accepts Clerk's session identifier (`sid`) as the
token ID when `jti` is absent, and accepts Clerk's machine authorized-party
claim (`azp`) as the service client identifier when `client_id` is absent. It
continues to enforce issuer, exact audience, algorithm, expiry, not-before,
and required token type.

No service token is minted in browser code. The worker obtains or reuses a
short-lived Clerk M2M JWT using a Secret Manager-held machine credential. App
API and Foundry reject tenant tokens on service routes and service tokens on
...
2. Worker obtains a Clerk M2M token from its machine credential, caching only
   until expiry.
3. Worker calls App API/Foundry with the matching service audience and scope.
4. Services verify issuer/audience/client identity and apply route-level service
   authorization.
...
- `CLERK_MACHINE_SECRET_KEY` for worker M2M issuance, if required by selected
  Clerk token flow.
...
- Verifier tests: issuer, exact audience, JWKS signature, `sid` fallback, `azp`
  fallback, expiry, not-before, token type, and cross-audience rejection.
...
- Worker tests: M2M token audience/scope, cache-before-expiry, expired-token
  refresh, and service-route rejection.
- Full Phase 1B source verifier and private deployment health gates remain
  required.

## Explicitly Deferred Decisions

- Clerk instance/domain and production credentials.
