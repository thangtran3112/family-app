...
└────────┬────────────┘
         ...
         ▼
┌─────────────────────┐
│  OCR Extraction     │   (Sync for MVP in backend/app/services/ocr.py)
...
└────────┬────────────┘
         ...
         ▼
┌─────────────────────┐
│  Post-Processing    │   (backend/app/services/processor.py)
...
### 1. File Upload API (`backend/app/api/v1/expenses.py`)
...
### 2. OCR & AI Extraction Service (`backend/app/services/ocr.py`)
...
- [ ] Implement **Monthly LLM Budget Guard** (`backend/app/services/budget_guard.py`):
...
- [ ] Define Pydantic v2 schemas (`backend/app/schemas/ocr.py`):
  ```python
