...
Run: `cd common/python/expense-contracts && uv run pytest -q`
...
Run: `cd common/python/expense-contracts && uv run pytest -q && uv run ruff check src tests`
...
```json
{
  ...
  "expense-service:test": "cd expense-service && uv run pytest"
}
```
...
Run: `cd expense-service && uv run pytest tests/test_shared_contracts.py -q`
...
Run: `cd expense-service && uv run pytest tests/test_shared_contracts.py -q && uv run ruff check .`
