...
const nonDatabaseGates = [
  ...
  {
    label: "Wave 1 App API route tests",
    command: "pnpm",
    args: ["--filter", "@expense-tax/app-api", "test"],
  },
];

for (const gate of nonDatabaseGates) runGate(gate);

let skippedDatabaseGate = false;
if (process.env.PHASE_0J_INTEGRATION === "1") {
  runGate({
  ...
  runGate({
    label: "Wave 1 App API migration",
    command: composeScript,
  ...
  runGate({
    ...
    args: [
      ...
      "vitest",
      "run",
      "test/integration/app-domain-wave1.test.ts",
    ],
  ...
  console.error(
    "\nSKIP Wave 1 PostgreSQL integration: PHASE_0J_INTEGRATION is not 1",
  );
}
...
if (skippedDatabaseGate) {
  console.error(
    "FAIL Phase 0J Wave 1 completion: required PostgreSQL checks were skipped",
  );
  process.exit(1);
}

console.log(
  "\nPASS Phase 0J Wave 1 aggregate verification: zero required checks skipped",
);
