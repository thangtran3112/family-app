...
runGate({
  label: "Phase 0I and Wave 1-2 regression verification",
  command: "pnpm",
  args: ["verify:phase-0j:wave2"],
  env: {
    ...process.env,
...
for (const gate of [
  {
    label: "Wave 3 and Wave 4 contract tests",
    command: "pnpm",
    args: [
      "--filter",
      ...
      "vitest",
      "run",
      "test/wave3-contracts.test.ts",
      "test/generated-artifacts.test.ts",
    ],
  },
  {
    label: "App API Wave 3 and Wave 4 tests",
    command: "pnpm",
...
if (
  process.env.PHASE_0J_WAVE3_INTEGRATION === "1" &&
  process.env.PHASE_0J_WAVE4_INTEGRATION === "1"
) {
  runGate({
  ...
  runGate({
    label: "Wave 3 PostgreSQL integration",
    command: "pnpm",
    args: ["exec", "vitest", "run", "test/integration/app-domain-wave3.test.ts"],
    env: { ...process.env, PHASE_0J_WAVE3_INTEGRATION: "1" },
  });
  runGate({
    label: "Wave 4 PostgreSQL integration",
    command: "pnpm",
    args: ["exec", "vitest", "run", "test/integration/app-domain-wave4.test.ts"],
    env: { ...process.env, PHASE_0J_WAVE4_INTEGRATION: "1" },
  });
} else {
  skippedDatabaseGate = true;
  console.error(
    "\nSKIP Wave 3/4 PostgreSQL integration: PHASE_0J_WAVE3_INTEGRATION and PHASE_0J_WAVE4_INTEGRATION must both be 1",
  );
}
...
if (skippedDatabaseGate) {
  console.error("FAIL Phase 0J Wave 4 completion: required PostgreSQL checks were skipped");
  process.exit(1);
}

console.log("\nPASS Phase 0J Waves 1-4 aggregate verification: zero required checks skipped");
