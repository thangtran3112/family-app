...
- Phase 1B durable deployment policy: GCP project `expense-tax-tobytran-2026` (organization `177410718350`, billing account `013C6D-EEE26-EAA1A1`) owns Secret Manager secret `expense-tax-production-env`; synchronization retains exactly one non-destroyed version and destroys older enabled/disabled versions after verification. GCP hosts Secret Manager, IAM, and GitHub OIDC/WIF only; no GCP VM is provisioned.
