...
- Phase 1B durable deployment policy: GCP project `expense-tax-tobytran-2026` (organization `177410718350`, billing account `013C6D-EEE26E-EAA1A1`) owns Secret Manager secret `expense-tax-production-env`; synchronization retains exactly one non-destroyed version and destroys older enabled/disabled versions after verification. GCP hosts Secret Manager, IAM, and GitHub OIDC/WIF only; no GCP VM is provisioned.
- Compute placement policy: keep always-on APIs, Temporal, workers, and stateful orchestration on existing VPS; use GCP Functions for future short-lived event-driven work; consider Cloud Run only by explicit case-by-case decision; do not place current Phase 1B services on GCP compute.
- Phase 1B source paths: `.github/workflows/expense-tax-deploy.yml` builds six immutable GHCR images after green `Expense Tax CI`; the workflow installs only `deploy/production/docker-compose.yml`, `deploy/production/deploy.sh`, and `deploy/production/health-check.sh` under `/opt/expense-tax-management/app`. Temporal database bootstrap remains an explicit operator-only Task 8 step and is never workflow-transferred. Application services bind VPS loopback only and database clients attach to external Docker network `postgres_default`; existing PostgreSQL remains outside application Compose.
- Remaining decisions before live/public use: tenant/platform identity provider and token claims, public hostnames, gateway/TLS/DNS, VPS-local versus GCS object storage, real OpenAI/OpenRouter provider activation and spend limits, and case-specific placement of future serverless/non-time-sensitive services.
