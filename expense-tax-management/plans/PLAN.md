...
```
family-app/                            # Git root (renamed from expense-tax-management)
├── infrastructure/                  # Shared stack
│   ├── docker-compose.common.yml    # postgres (pgvector), neo4j, temporal, gateway
│   ├── docker-compose.yml           # wrapper: includes common + expense-tax-management overlay
│   ├── nginx/nginx.conf
...
│   │       ├── lib/
│   │       │   ├── api.ts          # Typed API client from FastAPI OpenAPI
│   │       │   └── auth.ts         # Auth state and token management
│   │       ├── public/             # PWA icons, manifest.json
│   │       └── package.json
│   ├── expense-service/            # Python 3.14 / 3.13 Expense Service & Workers
...
│   │   ├── pyproject.toml          # Dependencies (FastAPI, Temporal, Graphiti, expense-contracts)
│   │   └── Dockerfile              # Python expense-service container
...
└── <future-family-app>/            # Sibling app, reuses ../infrastructure
```
├── docs/                           # Architecture docs, ADRs
└── README.md
```
