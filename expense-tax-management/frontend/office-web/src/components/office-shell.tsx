...
import { DUPLICATE_REVIEW_UPDATED_EVENT, fetchDuplicateMatches } from "@/lib/api";
...
export function OfficeShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    const refreshPendingCount = () => fetchDuplicateMatches(session, getToken, organization.id)
    ...
    return () => { active = false; window.removeEventListener(DUPLICATE_REVIEW_UPDATED_EVENT, refreshPendingCount); };
